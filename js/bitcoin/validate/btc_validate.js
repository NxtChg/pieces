...
{
	var i, j, b = [0], pos, carry; if(!s.length) return false;

	for(i = 0; i < s.length; i++)
	{
		pos = '123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz'.indexOf(s[i]); if(pos < 0) return false;
	
		for(j = 0; j < b.length; j++) b[j] *= 58;

		b[0] += pos; carry = 0;

		for(j = 0; j < b.length; j++){ b[j] += carry; carry = b[j] >> 8; b[j] &= 0xFF; }
		
		while(carry){ b.push(carry & 0xFF); carry >>= 8; }
	}
	
	for(i = 0; s[i] === '1' && i < s.length - 1; i++) b.push(0); // deal with leading zeros
...
{
	var b = base58_decode(adr);

	if(b !== false && b.length == 25)
	{
		var checksum = SHA256(SHA256(b.slice(0, b.length-4))).slice(0,4);

		if(checksum+'' == b.slice(b.length-4)+'' && (b[0] == 0 || b[0] == 5))
		{
			return true;
		}
	}
